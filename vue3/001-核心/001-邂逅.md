...
同时Vue提供的功能被拆分为多个库，可以根据需求逐步引入。你可以只使用它的一部分（如数据绑定和模板语法），也可以将其用作构建整个单页应用程序（SPA）的完整框架。
...
<body>
  <!--
		vue会将渲染后的内容挂载到挂载点下，而不会挂载到别的元素下
		挂载点下的元素可以通过vue来进行开发，而其它元素使用原生方式进行开发或使用React来进行开发
		这就是vue渐进式的一种体现形式
	-->
  <header>header</header>
  <!-- 挂载点 => 挂载点本身不是应用的一部分，挂载点中的内容才是应用 -->
  <div id="app"></div>
  <footer>footer</footer>

  <script>
    // 通过createApp方法创建对应App实例 => 参数可以是组件配置对象，或导入的组件
    // => 最终都会转换为经过优化的标准组件配置对象 「 例如模板会转换为render函数 」
    const app = Vue.createApp({
      // template是我们所需要渲染的界面模板 => 值一般为html格式字符串
      template: `<div>Hello Vue</div>`
    })

    // 挂载应用「 渲染应用 」 => 参数为字符串类型css字符串或者DOM元素
    app.mount('#app')
  </script>
</body>
</html>
